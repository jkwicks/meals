...
**v0.39.0**. Eleven releases have now closed whatever this queue ranked first
...
reasoning captured in prose and no entry anywhere. Listed so the queue is
complete.

**Five of the original six have now closed — two in v0.37.0, three in
v0.39.0 — and the appendix's own framing is what each closure corrected.**
...
|---|---|---|---|
| No daily fibre target | Feature | M | `fiber_g` is reported everywhere and budgeted nowhere, deliberately — it has no term in `calories ≈ 4p + 4c + 9f`. A real target needs a term in `calculate_macro_targets` and a per-slot share in `split_targets`. Displaying `32/xx` today would invent a goal the planner never aimed at. **Deliberately left open in v0.39.0** while its three neighbours closed: it is the one row here that has to change what a macro budget *is*, and Fast 800's ceiling closing beside it is not a precedent for it — a ceiling bounds a number the arithmetic already has, where fibre has no term to bound. |
